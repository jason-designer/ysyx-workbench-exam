...
static word_t immR(uint32_t i) { return 0; }
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  //jump
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = 4 + s->pc; s->dnpc = s->pc + src2);
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, word_t t = s->pc + 4; s->dnpc = (src1 + src2) & (~(word_t)1); R(dest) = t);
