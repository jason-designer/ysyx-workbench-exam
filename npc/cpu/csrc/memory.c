...
uint64_t load_program(char* img_file){
    ...
    // 无现关性冲突的跳转用例----------------------
    m[0]  = 0x00000093;     //li  ra,0
    m[1]  = 0x00100113;     //li  sp,1
    m[2]  = 0x00200193;     //li  gp,2
    m[3]  = 0x00000393;     //li  t2,0
    m[4]  = 0x00000513;     //li  a0,0
    m[5]  = 0x00000513;     //li  a0,0
    m[6]  = 0x00209663;     //bne	ra,sp,pc + 0xc 
    m[7]  = 0x00100513;     //li  a0,1
    m[8]  = 0x00200513;     //li  a0,2
    m[9]  = 0x00300513;     //li  a0,3
    m[10] = 0x00400513;     //li  a0,4
    m[11] = 0x00500513;     //li  a0,5
    m[12] = 0x00600513;     //li  a0,6
    m[13] = 0x00000513;     //li  a0,0
    m[14] = 0x00100073;     //ebreak
    return 60;              // built-in image size
...
void pmem_read(unsigned char ren, long long raddr, long long *rdata) {
    ...
    if(ren) {
        if(raddr >= DEVICE_BASE){
            mmio_read_device(raddr, rdata);
            return;
        }

        *rdata = read_memory(raddr & 0xfffffffffffffff8, 8);
    }
}
